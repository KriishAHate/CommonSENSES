...
data = read.csv('/Users/kriishhate/Desktop/Dr. Mueller Lab Work/Sound sensor test 29:10:2024 /Processed files/20241029_big_hole.csv')
...
data$Datetime = as.POSIXct(paste(data$YYYYMMDD, data$HHMMSS), format = "%Y%m%d %H%M%S")
...
for (bin_start in proper_time_scale[-length(proper_time_scale)]) {
    ...
    bin_data = data[data$Datetime >= bin_start & data$Datetime < bin_end, ]
...
knitr::opts_chunk$set(echo = TRUE)
# Load the dataset from the specified file path
data_small_hole = read.csv('/Users/kriishhate/Desktop/Dr. Mueller Lab Work/Sound sensor test 29:10:2024 /Processed files/20241029_small_hole.csv')

# Combine the date and time columns into a single column named 'Datetime'
# Convert the combined column into a POSIXct datetime format for time-based operations
data_small_hole$Datetime = as.POSIXct(paste(data_small_hole$YYYYMMDD, data_small_hole$HHMMSS), format = "%Y%m%d %H%M%S")

# Define the size of each time interval in seconds
interval = 5

# Find the earliest timestamp in the dataset and assign it to 'start_time'
start_time = min(data_small_hole$Datetime)

# Find the latest timestamp in the dataset and assign it to 'end_time'
end_time = max(data_small_hole$Datetime)
...
for (bin_start in proper_time_scale[-length(proper_time_scale)]) {
    ...
    # Filter the dataset to include only rows with timestamps within the current bin
    bin_data = data_small_hole[data_small_hole$Datetime >= bin_start & data_small_hole$Datetime < bin_end, ]
...
knitr::opts_chunk$set(echo = TRUE)
# Load the dataset from the specified file path
data = read.csv('/Users/kriishhate/Desktop/Dr. Mueller Lab Work/Sound sensor test 29:10:2024 /Processed files/20241029_open.csv')
...
# Print the proper time scale to verify the generated intervals
print("Proper Time Scale:")
print(proper_time_scale)
