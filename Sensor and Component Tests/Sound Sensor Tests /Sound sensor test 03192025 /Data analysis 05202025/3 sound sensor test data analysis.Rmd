...
```{r PLot 6-1 open vs Big hole dB readings using index as binning instead of time, include=TRUE}


# Set bin size
bin_size <- 100

# Calculate total number of full bins
n_bins <- floor(nrow(data) / bin_size)
...
for (i in 1:n_bins) {
  ...
  bin_rows <- data[start_index:end_index, ]
  
  binned_Sensor1_dB[i] <- mean(bin_rows$Sensor1_dB, na.rm = TRUE)
  binned_Sensor3_dB[i] <- mean(bin_rows$Sensor3_dB, na.rm = TRUE)
}

# Create binned data frame
binned_dB_big_df <- data.frame(
  Bin = 1:n_bins,
  Sensor1_dB = binned_Sensor1_dB,
  Sensor3_dB = binned_Sensor3_dB
...
# Regression
model_dB_big <- lm(Sensor3_dB ~ Sensor1_dB, data = binned_dB_big_df)
slope <- coef(model_dB_big)[2]
intercept <- coef(model_dB_big)[1]
r_squared <- summary(model_dB_big)$r.squared
correlation <- sqrt(r_squared)
...
text_x <- min(binned_dB_big_df$Sensor1_dB) + 0.05 * diff(range(binned_dB_big_df$Sensor1_dB))
text_y <- min(binned_dB_big_df$Sensor3_dB) + 0.85 * diff(range(binned_dB_big_df$Sensor3_dB))
...
plot_dB_big <- ggplot(binned_dB_big_df, aes(x = Sensor1_dB, y = Sensor3_dB)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  annotate("text", x = text_x, y = text_y, label = regression_text, size = 5, hjust = 0) +
  ggtitle(paste("Sensor 1 (Open) vs Sensor 3 (Big Hole) - dB (Binned Every", bin_size, "Samples)")) +
  xlab("Sensor 1 (dB)") +
  ylab("Sensor 3 (dB)") +
  theme_minimal()
